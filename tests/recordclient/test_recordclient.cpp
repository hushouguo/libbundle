...
void test_serialize(const char* address, int port) {
...
void test_unserialize(const char* address, int port) {
	RecordClient recordClient;
	bool rc = recordClient.connect(address, port);
	CHECK_RETURN(rc, void(0), "unreachable recordserver: %s,%d", address, port);
	...
	Time t1;
	for (u32 n = 0; n < count; ++n) {
		recordClient.unserialize(shard, table, n, 
				[&recordClient, &res_ok, &res_fail, count](u32 shard, std::string table, u64 objectid, u32 retval, const char* data, size_t datalen) {
					//std::ostringstream o;
	...
	Time t2;
	Trace << "benchmark: unserialize " << count << " objects, cost milliseconds: " << t2 - t1;
	Trace << "	  Success: " << res_ok << ", Fail: " << res_fail;
}

void test_recordclient(const char* address, int port) {
	test_serialize(address, port);
	test_unserialize(address, port);
